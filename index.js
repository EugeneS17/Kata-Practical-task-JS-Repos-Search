class View {
    ...
    this.selectedReposList = this.selectedRepos.querySelector('.selected-repos__list');
...
addEventListeners() {
    this.searchInput.addEventListener('keyup', this.searchInputHandler.bind(this));

    this.resultList.addEventListener('click', this.resultListHandler.bind(this));

    this.selectedReposList.addEventListener('click', this.selectedReposListHandler.bind(this));
}
...
setSelectedRepos(repo) {
    ...
    localStorage.setItem('selectedRepos', JSON.stringify(this.selectedReposVault));
}
...
createRepoCard(repo) {
    const element = this.createElement('li', [
        'selected-repos__list-item', 'repo-card',
    ]);
